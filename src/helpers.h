...
inline double polyeval(const VectorXd &coeffs, double x) {
  double result = 0.0;
...
inline VectorXd polyfit(const VectorXd &xvals, const VectorXd &yvals, int order) {
...
scalar_t cppadPolyeval(Eigen::VectorXd coeffs, scalar_t x, bool bSlope) {
  ...
  if(!bSlope){
    ...
    // Calculate the slope of the polynomial
